...
pub const LLVMCodegen = struct {
    ...
    pub fn generate(self: *LLVMCodegen, root: *ast.Node) ![]const u8 {
        ...
        try self.external_decls.put("mantiq_arena_alloc", true);
        try preamble.writer().print("declare void @mantiq_arena_reset(ptr)\n", .{});
        try self.external_decls.put("mantiq_arena_reset", true);
        try preamble.writer().print("declare void @mantiq_arena_destroy(ptr)\n", .{});
        try self.external_decls.put("mantiq_arena_destroy", true);
